...
//   monopolyKartenspiel
// } from 'monopoly.js';
const vierGewinntTeams = require('./vierGewinntTeams');
...
var vierGewinntTeamsInstance = new vierGewinntTeams(addBroadcast, addMessage);
...
app.get('/', function(req, res) {
  res.sendFile('static/vierGewinntTeams.html', {
    root: __dirname
  });
});
...
//  console.log(messageQueue);
  vierGewinntTeamsInstance.onJoin({
    sessionId: clientId
  });
});
app.post('/logout', function(req, res) {
  ...
  vierGewinntTeamsInstance.onLeave({
    sessionId: clientId
  });
});
...
app.post('/message', function(req, res) {
  vierGewinntTeamsInstance.onMessage(req.body.clientId, req.body.content)
  res.json({
    success: true
  });
});
